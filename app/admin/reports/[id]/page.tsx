...
export default function ReportDetailPage() {
  ...
  function startEdit(p: Project) {
    ...
    setForm({
      ...
      completion_pct: p.completion_pct.toString(),
      status:         p.status,
  ...
  async function handleDelete(id: string, name: string) {
    ...
    else { fetchData(); setSelectedProject(null); }
  }
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="date" value={form.sop_date}
                onChange={e => setForm({ ...form, sop_date: e.target.value })}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Volume (units/month)</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Completion % (0-100)</label>
              <input type="number" min="0" max="100" value={form.completion_pct}
                onChange={e => setForm({ ...form, completion_pct: e.target.value })}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
            ...
            <div className="md:col-span-3">
              ...
              <textarea value={form.summary_text}
                onChange={e => setForm({ ...form, summary_text: e.target.value })}
                rows={2} placeholder="On Track: Within budget, scope, schedule, and risk expectations."
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div className="flex gap-3 mt-4">
            <button onClick={handleSave} disabled={saving}
              className="px-4 py-2 bg-blue-950 text-white rounded-lg text-sm font-semibold hover:bg-blue-900 transition-colors disabled:opacity-50">
              ...
              <div className="p-5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    {selectedProject?.id === p.id ? '▲ Hide' : '▼ Action Items'} ({(p.action_items ?? []).length})
                  </button>
...
function ActionItemsPanel({ project, onRefresh }: { project: Project; onRefresh: () => void }) {
  ...
  function startNew() {
    setEditingId(null);
    setForm({ item_no: '', item_category: '', issue_desc: '', action_plan: '', completion_pct: '0', due_date: '', is_info_only: false });
    setShowForm(true);
  }
  ...
  function startEdit(item: NonNullable<Project['action_items']>[0]) {
    ...
    setForm({
      ...
      completion_pct: item.completion_pct.toString(),
    ...
    <div className="border-t border-slate-100 bg-slate-50 p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={startNew}
          className="px-3 py-1 bg-blue-950 text-white rounded-lg text-xs font-semibold hover:bg-blue-900 transition-colors">
          + Add Item
        </button>
      </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-4 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" value={form.item_no}
            ...
            <div className="md:col-span-2">
              ...
              <textarea value={form.action_plan}
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="info_only" checked={form.is_info_only}
                onChange={e => setForm({ ...form, is_info_only: e.target.checked })}
                className="w-4 h-4" />
              <label htmlFor="info_only" className="text-xs text-slate-600">Info only (no progress tracking)</label>
            </div>
          </div>
      ...
      {items.length === 0 ? (
        <p className="text-xs text-slate-400 text-center py-4">No action items yet. Click + Add Item above.</p>
      ) : (
            ...
            <div key={item.id} className="bg-white rounded-lg border border-slate-200 px-4 py-3 flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  {!item.is_info_only && (
                    <span className="text-xs font-semibold text-slate-600">{item.completion_pct}%</span>
                  )}
                  {item.due_date && (
