...
export default function AdminTimesheet() {
  ...
  const [factoryForm, setFactoryForm] = useState({ code: '', name: '', sort_order: '', address: '', registration_no: '', phone: '', email: '', bank_name: '', bank_account: '', bank_account_name: '', sst_rate: '0', payment_terms: '30 days' });
  ...
  async function handleSaveFactory() {
    ...
    setSaving(true);
    const payload = { code: factoryForm.code.toUpperCase(), name: factoryForm.name, sort_order: parseInt(factoryForm.sort_order) || 0, address: factoryForm.address || null, registration_no: factoryForm.registration_no || null, phone: factoryForm.phone || null, email: factoryForm.email || null, bank_name: factoryForm.bank_name || null, bank_account: factoryForm.bank_account || null, bank_account_name: factoryForm.bank_account_name || null, sst_rate: parseFloat(factoryForm.sst_rate) || 0, payment_terms: factoryForm.payment_terms || '30 days' };
    if (editingFactoryId) {
      await supabase.from('ts_factories').update(payload).eq('id', editingFactoryId);
    } else {
      await supabase.from('ts_factories').insert([payload]);
    }
    setShowFactoryForm(false); setEditingFactoryId(null); setFactoryForm({ code: '', name: '', sort_order: '', address: '', registration_no: '', phone: '', email: '', bank_name: '', bank_account: '', bank_account_name: '', sst_rate: '0', payment_terms: '30 days' });
    fetchData(); setSaving(false);
  }
        ...
        {activeTab === 'factories' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setEditingFactoryId(null); setFactoryForm({ code: '', name: '', sort_order: '', address: '', registration_no: '', phone: '', email: '', bank_name: '', bank_account: '', bank_account_name: '', sst_rate: '0', payment_terms: '30 days' }); setShowFactoryForm(true); }}
            ...
            {showFactoryForm && (
              <div className="bg-white rounded-xl border border-blue-200 p-5 mb-4">
                ...
                <div className="space-y-4">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b pb-2">Basic Info</p>
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" value={factoryForm.name} onChange={e => setFactoryForm({ ...factoryForm, name: e.target.value })}
                        placeholder="Full company name" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Registration No.</label>
                      <input type="text" value={factoryForm.registration_no} onChange={e => setFactoryForm({ ...factoryForm, registration_no: e.target.value })}
                        placeholder="e.g. 277264-H" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    <div className="col-span-2">
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Address</label>
                      <textarea value={factoryForm.address} onChange={e => setFactoryForm({ ...factoryForm, address: e.target.value })}
                        rows={3} placeholder="Full address" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Phone</label>
                        <input type="text" value={factoryForm.phone} onChange={e => setFactoryForm({ ...factoryForm, phone: e.target.value })}
                          placeholder="e.g. +603-8926 3610" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Email</label>
                        <input type="email" value={factoryForm.email} onChange={e => setFactoryForm({ ...factoryForm, email: e.target.value })}
                          placeholder="e.g. finance@stcsb.com" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </div>
                    </div>
                  </div>
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b pb-2 mt-2">Banking Details</p>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Bank Name</label>
                      <input type="text" value={factoryForm.bank_name} onChange={e => setFactoryForm({ ...factoryForm, bank_name: e.target.value })}
                        placeholder="e.g. Maybank" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Account Number</label>
                      <input type="text" value={factoryForm.bank_account} onChange={e => setFactoryForm({ ...factoryForm, bank_account: e.target.value })}
                        placeholder="e.g. 5142 0123 4567" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Account Name</label>
                      <input type="text" value={factoryForm.bank_account_name} onChange={e => setFactoryForm({ ...factoryForm, bank_account_name: e.target.value })}
                        placeholder="e.g. SAPURA TECHNICAL CENTRE" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  </div>
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b pb-2 mt-2">Invoice Settings</p>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">SST Rate (%)</label>
                      <input type="number" step="0.1" min="0" max="100" value={factoryForm.sst_rate} onChange={e => setFactoryForm({ ...factoryForm, sst_rate: e.target.value })}
                        className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Payment Terms</label>
                      <select value={factoryForm.payment_terms} onChange={e => setFactoryForm({ ...factoryForm, payment_terms: e.target.value })}
                        className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <tbody>
                  {factories.map((f, i) => (
                    <tr key={f.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                      ...
                      <td className="py-2 px-4">
                        <div className="flex gap-1">
                          <button onClick={() => { setEditingFactoryId(f.id); setFactoryForm({ code: f.code, name: f.name, sort_order: f.sort_order.toString(), address: f.address ?? '', registration_no: f.registration_no ?? '', phone: f.phone ?? '', email: f.email ?? '', bank_name: f.bank_name ?? '', bank_account: f.bank_account ?? '', bank_account_name: f.bank_account_name ?? '', sst_rate: f.sst_rate?.toString() ?? '0', payment_terms: f.payment_terms ?? '30 days' }); setShowFactoryForm(true); }}
                            className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs hover:bg-blue-100">✏️</button>
