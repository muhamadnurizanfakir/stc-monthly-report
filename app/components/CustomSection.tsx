      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ background: sectionColor }}>
              <span className="text-white font-bold text-xs">{project.project_code || "#"}</span>
            </div>
            <div>
              <h3 className="font-bold text-slate-800">{project.project_name}</h3>
              {project.customer && <p className="text-xs text-slate-500 mt-0.5">{project.customer}{project.model ? " · " + project.model : ""}</p>}
            </div>
          </div>
      ...
      {expanded && (
        <div className="border-t border-slate-200">
          <div className="p-4 bg-slate-50 border-b border-slate-200">
            ...
            <GanttChart customProjectId={project.id} />
          </div>
          {items.length > 0 && (
            <div>
                ...
                <tbody>
                  {items.map((item, i) => (
                    <tr key={item.id} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                      ...
                      <td className="px-4 py-2.5 text-center">{item.is_info_only ? <span className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded">Info</span> : <span className="font-semibold text-slate-700">{item.completion_pct}%</span>}</td>
                      <td className="px-4 py-2.5 text-center text-slate-500">{item.due_date ? new Date(item.due_date).toLocaleDateString("en-MY", { day: "2-digit", month: "short", year: "2-digit" }) : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="p-5 border-b border-slate-100">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <h3 className="font-bold text-slate-800 mb-3">{section.name} Projects</h3>
        ...
        <div className="mt-4 flex items-center gap-3">
          <button onClick={() => setExpanded(!expanded)}
            className="text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors" style={{ background: section.color + "15", color: section.color }}>
            {expanded ? "▲ Hide Details" : "▼ Show Gantt & Action Items"}
          </button>
          {hidden.length > 0 && (
            <button onClick={() => setShowHidden(!showHidden)}
              className="text-xs font-semibold px-3 py-1.5 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-lg">
      ...
      {expanded && (
        <div>
          <div className="p-4 bg-slate-50 border-b border-slate-200">
            <p className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-3">Gantt Chart</p>
            <GanttChart sectionId={section.id} />
          </div>
          {allItems.length > 0 && (
            <div>
              <div className="px-4 py-2 bg-slate-100 border-b border-slate-200">
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {allItems.map((item, i) => (
                    <tr key={item.id} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                      <td className="px-4 py-2.5 text-slate-400">{item.item_no}</td>
                      <td className="px-4 py-2.5 text-slate-600 font-medium">{item.project_name}</td>
                      <td className="px-4 py-2.5 text-slate-700">{item.issue_desc}</td>
                      <td className="px-4 py-2.5 text-slate-600">{item.action_plan}</td>
                      <td className="px-4 py-2.5 text-center">{item.is_info_only ? <span className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded">Info</span> : <span className="font-semibold text-slate-700">{item.completion_pct}%</span>}</td>
                      <td className="px-4 py-2.5 text-center text-slate-500">{item.due_date ? new Date(item.due_date).toLocaleDateString("en-MY", { day: "2-digit", month: "short", year: "2-digit" }) : "-"}</td>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-2xl text-slate-800">{section.icon} {section.name}</h2>
            <p className="text-slate-500 text-xs">{visible.length} project{visible.length !== 1 ? "s" : ""} tracked</p>
          </div>
        </div>
        {hidden.length > 0 && (
          <button onClick={() => setShowHidden(!showHidden)}
