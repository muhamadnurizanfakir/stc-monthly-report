...
export default function ShohinSection({ shohinProjects, onRefresh }: Props) {
  ...
  const display = showHidden ? shohinProjects : visible;
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">◉ Process Improvement</h1>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {project.category && (
                          <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full text-xs">{project.category}</span>
                        )}
                      </div>
                      <div className="flex flex-wrap gap-3 text-xs text-slate-500">
                        {project.customer && <span>👤 {project.customer}</span>}
                        {(project as {model?: string|null}).model && <span>🚗 {(project as {model?: string|null}).model}</span>}
                        {project.sop_date && <span>📅 SOP: {new Date(project.sop_date).toLocaleDateString('en-MY', { month: 'short', year: 'numeric' })}</span>}
                        {(project as {volume?: number|null}).volume && <span>📦 {((project as {volume?: number|null}).volume ?? 0).toLocaleString()} units</span>}
                      </div>
                    </div>
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                    <button onClick={() => setExpandedActions(expandedActions === project.id ? null : project.id)}
                      ...
                      {expandedActions === project.id ? '▲ Hide' : '▼ Actions'} ({(project.shohin_action_items ?? []).length})
                    </button>
