...
import type { OthersProject } from '../lib/supabase';

interface Props {
  projects: OthersProject[];
}
...
export default function OthersSection({ projects }: Props) {
      ...
      {projects.length === 0 ? (
        ...
        <div className="space-y-4">
          {visible.map(project => {
            ...
            const actionItems = (project.others_action_items ?? []);
            return (
              <div key={project.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-5">
                  ...
                  {expandedGantt === project.id && (
                    <div className="mt-4 border-t border-slate-100 pt-3">
                      <GanttEditor othersProjectId={project.id} projectName={project.project_name} />
                    </div>
                  )}
